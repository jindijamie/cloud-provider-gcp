...
func TestValidateAuthFlow(t *testing.T) {
	...
	tests := []FlagResult{
		...
		{Name: "identity-provider with gcr flow", Options: CredentialOptions{AuthFlow: gcrAuthFlow, IdentityProvider: "https://container.googleapis.com/..."}},
		{Name: "identity-provider with dockercfg flow", Options: CredentialOptions{AuthFlow: dockerConfigAuthFlow, IdentityProvider: "https://container.googleapis.com/..."}},
		{Name: "identity-provider with dockercfg-url flow", Options: CredentialOptions{AuthFlow: dockerConfigURLAuthFlow, IdentityProvider: "https://container.googleapis.com/..."}},
	}
...
func TestProviderFromFlow(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.Name, func(t *testing.T) {
			provider, err := providerFromFlow(tc.Flow, credentialproviderapi.CredentialProviderRequest{}, "")
			if tc.Error != nil {
				if err == nil {
...
func TestFlowError(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.Name, func(t *testing.T) {
			_, err := providerFromFlow(tc.Flow, credentialproviderapi.CredentialProviderRequest{}, "")
			if !errors.Is(err, &tc.ExpectedError) {
				t.Fatalf("did not get expected error %q (got %q instead", &tc.ExpectedError, err)
			}
			if !strings.Contains(err.Error(), tc.MessageContains) {
