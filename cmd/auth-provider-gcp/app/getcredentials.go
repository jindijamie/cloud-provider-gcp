...
func providerFromFlow(flow string, req credentialproviderapi.CredentialProviderRequest, identityProvider string) (credentialconfig.DockerConfigProvider, error) {
	transport := utilnet.SetTransportDefaults(&http.Transport{})
	switch flow {
	case gcrAuthFlow:
		return provider.MakeRegistryProvider(transport, req.ServiceAccountToken, req.ServiceAccountAnnotations, identityProvider), nil
...
func getCredentials(options CredentialOptions) error {
	...
	authProvider, err := providerFromFlow(options.AuthFlow, authRequest, options.IdentityProvider)
	if err != nil {
		return err
	}
